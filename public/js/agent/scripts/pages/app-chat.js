...
$(function () {
  ...
  if(selectChatUser.length) {
    selectChatUser.click(function(e) {
      $('#sender_id').val($(this).attr('data-id'));
      $('#select-user-name').html($(this).attr('data-name'));
      if($(this).attr('data-image')) {
        $('#select-user-pic').attr('src', $(this).attr('data-image'));
      }
      $('#doPCall').attr('href', $(this).attr('data-phone'));
      $('#viewProfile').attr('href', '/agent/profile/' + $(this).attr('data-name') + '/' + $(this).attr('data-id'));
      markChatRead($(this).attr('data-id'));
    });
  }
...
function enterChat(source) {
  ...
  if (/\S/.test(message)) {
    ...
    var html = '<div class="chat"><div class="chat-avatar"><span class="avatar box-shadow-1 cursor-pointer"><img src="/uploads/profiles/'+ userImage +'" alt="avatar" height="36" width="36" onerror="this.onerror=null;this.src="https://via.placeholder.com/150?text=No Image";"></span></div><div class="chat-body"><div class="chat-content"><p>'+ message +'</p></div></div></div>';
    $('.chats').append(html);
    $('.message').val('');
...
function markChatRead(sender) {
  $.ajax({
    url : '/api/agent/message/read',
    type: 'POST',
    dataType: 'JSON',
    ...
    success: function(response) {
      if(response.messages.length) {
        var html = '';
        for(var c = 0; c < response.messages.length; c++) {
                userImage = response.user_pic;
                html = html + '<div class="divider"><div class="divider-text">'+ response.messages[c].date +'</div></div>';
                var chat = response.messages[c].chat;
                for(var i = 0; i < chat.length; i++) {
                        if(chat[i].sender_id != response.current_user) {
                          html = html + '<div class="chat-left">';
                        } else {
                          html = html + '<div class="chat">';
                        }
                           
                        html = html + '<div class="chat-avatar">'
                           + '<span class="avatar box-shadow-1 cursor-pointer">';
                        if(chat[i].sender_id != response.current_user) {
                          if(chat[i].sender_pic !='') {
                              html = html + '<img src="/uploads/profiles/'+ chat[i].sender_pic +'" alt="avatar" height="36" width="36" onerror="this.onerror=null;this.src="https://via.placeholder.com/150?text=No Image";"/>';
                          } else {
                              html = html + '<img src="https://via.placeholder.com/150?text=No Image" alt="avatar" height="36" width="36" onerror="this.onerror=null;this.src="https://via.placeholder.com/150?text=No Image";"/>';
                          }
                        } else {
                          html = html + '<img src="/uploads/profiles/'+ response.user_pic +'" alt="avatar" height="36" width="36" onerror="this.onerror=null;this.src="https://via.placeholder.com/150?text=No Image";"/>';
                        }
                         html = html + '</span></div><div class="chat-body">'
                           + '<div class="chat-content"><p>' + chat[i].message
                           + '</p></div></div></div>';
               }
