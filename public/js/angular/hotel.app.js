var app = angular.module("hotelApp", ['infinite-scroll', 'ngSanitize']);
...
app.controller("searchCtrl", function ($scope, $http, $timeout, $locale, $location, $rootScope, $window) {
    ...
    $scope.searchHotels = function (load) {
        ...
        if (load) {
            ...
            $scope.hotels = [];
            $('#autocomplete').val();
            queryParmas.push({'Location': $('#autocomplete').val()},
        ...
        $http.post('/api/hotels', queryParmas)
                .then(function (returnArray) {
                    ...
                    if (response.status) {
                        ...
                        $scope.highstPrice = response.highstPrice;
                        $scope.priceRange = response.highstPrice;
                    ...
                    setTimeout(function(){
                        ...
                        if (window.innerWidth > 767) {
                             $('.ListHotelOnLOadFilter').css('display', 'block');
                        }

                        $('.ListHotelOnLOad').css('display', 'block');
                        $('.ListHotelOnLOadFilter2').css('display', 'flex');
...
//        if($scope.r_amenities.length){
//            
//            $scope.hotels = $scope.hotels.filter((hotel) => {
//                var room_amets=[];
//                if(hotel.static_data.room_amenities.length > 0){
//                    room_amets=$scope.r_amenities.filter(x => hotel.static_data.room_amenities.includes(x));
//                }
//                return room_amets.length > 0;
//            });
//            
//        }
    ...
    $scope.loadMoreHotels = function () {
        ...
        $http.get('/api/hotels/more/' + $scope.hotels.length + '/' + $scope.search_id + '/' + $scope.priceRange + '/' + $scope.distanceRange + '?' + queryparm)
                .then(function (returnArray) {

                    var response = returnArray.data;
                    if (response.hotels && response.hotels.length) {
                        var hcount = $scope.hotels.length;
                        for (var i = 0; i < response.hotels.length; i++) {
                            // $scope.hotels.push(response.hotels[i]);
                            $scope.hotels.pushIfNotExist(response.hotels[i], function (e) {
                                return e.TBO_data.HotelCode === response.hotels[i].TBO_data.HotelCode;
                            });
                        }
                        var nhcount = $scope.hotels.length;
                        // $scope.hotels.sort(function(a, b) { return b.TBO_data.h_rating - a.TBO_data.h_rating; });
                        $scope.loadMap();
                        $scope.busy = false;
                        $scope.loadingMore = true;
        ...
        for (i = 0; i < locations.length; i++) {
            ...
            google.maps.event.addListener(marker, 'click', (function (marker, i) {
                ...
                return function () {
                    ...
                    var hotelLink = '/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + locations[i].code + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest;
...
//        });
    ...
    $scope.viewHotel = function (e, hotel, referral) {
        if ($(e.target).closest(".carousel-indicators").length || $(e.target).closest(".list-inline-item").length || $(e.target).closest(".email-hotel-check").length || $(e.target).closest(".share-post-icons").length) {
        } else {
            referral = referral || '0';
            ...
            $window.open('/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + hotel.TBO_data.HotelCode + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest, '_blank');
        }
    }
    ...
    $scope.copyToClip = function (e, hotel, referral) {
        ...
        var link = domain + '/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + hotel.TBO_data.HotelCode + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest;
        var $temp = $("<input>");
        $("body").append($temp);
    ...
    $scope.viewHotelFB = function (e, hotel, referral, type) {
        if ($(e.target).closest(".carousel-indicators").length || $(e.target).closest(".list-inline-item").length || $(e.target).closest(".email-hotel-check").length) {
        } else {
            referral = referral || '0';
            ...
            if (type == 'fb') {
                ...
                $window.open('https://www.facebook.com/sharer/sharer.php?u=' + domain + '/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + hotel.TBO_data.HotelCode + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest + '&quote=' + hotel.static_data.hotel_name);
            } else if (type == 'wts') {

                $window.open('whatsapp://send?text=' + domain + '/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + hotel.TBO_data.HotelCode + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest);

            } else if (type == 'tw') {

                $window.open('http://twitter.com/share?text=' + hotel.static_data.hotel_name + '&url=' + domain + '/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + hotel.TBO_data.HotelCode + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest);

            } else if (type == 'pt') {

                $window.open('https://pinterest.com/pin/create/bookmarklet/?media=' + hotel.static_data.hotel_images + '&url=' + domain + '/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + hotel.TBO_data.HotelCode + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest + '+&description=' + hotel.static_data.hotel_name + '');

            } else if (type == 'stb') {

                $window.open('https://www.stumbleupon.com/submit?url=' + domain + '/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + hotel.TBO_data.HotelCode + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest + '&title=' + hotel.static_data.hotel_name + '');

            } else if (type == 'lkn') {

                $window.open('https://www.linkedin.com/shareArticle?url=' + domain + '/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + hotel.TBO_data.HotelCode + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest + '&title=' + hotel.static_data.hotel_name + '');

            } else if (type == 'inst') {

                $window.open('https://www.instagram.com/?url=' + domain + '/hotel/' + country_name + '/' + city_name + '/' + hotel_name + '/' + $scope.traceId + '/' + hotel.TBO_data.HotelCode + '/' + $('#departHotel').val() + '/' + $('#roomCount').val() + '/' + $scope.search_id + '/' + $scope.searchData.NoOfNights + '/' + referral + '?' + guest);
...
//        }
    ...
    $scope.sendSelectedHotels = function () {
        ...
        $http.post('/api/hotels-send', {email: $scope.hotelEmail, hotel: $scope.selectedHotel, search_id: $scope.search_id})
...
app.controller("roomCtrl", function ($scope, $http, $timeout, $locale, $location, $rootScope, $window, $filter) {
    ...
    var params = $location.absUrl().split('/hotel/');
    var queryParmas = [];
    params = params[1].split('/');
    $scope.priceRange = 500000;
    //$scope.roomCount = $('#roomCount').val();
    $scope.roomCount = [];
    for (var i = 0; i < $('#roomCount').val(); i++) {
        $scope.roomCount.push(i);
    }
    //console.log( $scope.roomCount);
    if (params.length) {
        var obj = {};
        obj['traceId'] = decodeURI(params[3]);
        obj['hotelCode'] = decodeURI(params[4]);
        ...
        $http.post('/api/hotel/rooms', obj)
                .then(function (returnArray) {
                    ...
                    if (response.status) {
                        ...
                        for (var r = 0; r < parseInt($('#roomCount').val()); r++) {
...
//    $scope.filterByPrice = function (value) {
//        $scope.loaded = false;
//        var selectRoomCount = $('#roomCount').val();
//
//        $('.rooms-tr-meal').each(function () {
